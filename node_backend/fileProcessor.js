...
event.on('process', async (data, filePath, title) => {
  ...
  for (const chunk of chunks) {
    const content = `Extract Model, Storage (GB), Lock Status, SIM Type, Device Type(iphone, samsung, laptop, watch, sound) and Highest and Lowest 3 prices from the text below and return the value as a list of json object with each object like 'model':'value', 'storage':'value', 'lock_status':'value', 'sim_type':'value', 'device_type':'value': 'H1':'value', 'H2':'value', 'H3':'value', 'L1':'value', 'L2':'value', 'L3':'value',. If a line contains more than one price specification, extract each price as different json object.
