    ...
    const content = `Extract Model, Storage (GB), Lock Status, SIM Type, Device Type(iphone, samsung, laptop, watch, sound, tablet) and Price from the text below and return the value as a list of json object with each object having the following keys 'model','storage','lock_status','sim_type’,’device_type’,’price'. 
		    ...
		    4. For laptops lock_status should be RAM and sim_type should be the processor 
	    	    ...
	    	    6. Make sure all tablet carries the generation and the size. e.g. iPad Pro is not a valid model, but iPad Pro 3rd Gen 12.9 inches is valid 
                    ...
                    10. Ensure that iphones are represented as iPhone, samsung are represente as Samsung, basically make sure that all product name and models are uniform
                    11. Always return a valid json object, no extra markdown character. Don't add these characters "\`\`\`json".
                    12. Ensure to always stick to the pattern without any deviation. Your response should not be in markdown. Send it to me as a direct string. Ensure to pass the right data to the right object key. Any value that has the word "unlocked" or “FU”,  must be in the lock_status key. Any value that contains the word "sim" must be in the sim_type key.
                    13. iPads should be be under tablet device type
                    14. Ensure that only iphones are under iphone type, only samsung phones are under samsung, only laptops (including macbooks) are under laptop, only watches are under watch, same for sound and tablet
                    15. For all model names, ensure that you consitently user the same name even if they look different from the provided data
                    16. For storage size, always specify the storage unit. e.g. 256GB, 1TB, 128GB.  `;
