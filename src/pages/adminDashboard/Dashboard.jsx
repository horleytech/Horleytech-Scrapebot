...
const AdminDashboard = () => {
  ...
  const exportPricingTxt = () => {
    const validItems = pricingResults.filter((item) => Number.isFinite(item.companyPrice)
      && item.companyPrice > 0
      && (((item.hasVendorMatch && item.hasDefaultMarginConfigured) || item.isOverridden)));
                                      ...
                                      <tbody>
                                        {subCat.items.map((row) => (
                                          <tr key={`pricing-${row.rowKey}`} className="border-t border-gray-50 text-sm hover:bg-blue-50/30 transition-colors">
                                            ...
                                            <td className="px-3 py-2 font-bold text-indigo-600">{((row.hasVendorMatch && row.hasDefaultMarginConfigured) || row.isOverridden) ? formatNaira(row.target) : 'N/A'}</td>
                                            <td className="px-3 py-2">
                                              {((row.hasVendorMatch && row.hasDefaultMarginConfigured) || row.isOverridden) ? (
