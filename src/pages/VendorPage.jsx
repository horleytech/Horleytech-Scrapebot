...
const VendorPage = () => {
  ...
  const handleSaveSettings = async () => {
    const cleanedNumbers = whatsappNumbersInput.map((n) => n.trim()).filter(Boolean).slice(0, 3);
  ...
  const saveInlineEdit = async () => {
    ...
    const nextProducts = products.map((product, index) => {
      ...
      return {
        ...
        'SIM Type/Model/Processor': editSpecification.trim(),
        'Storage Capacity/Configuration': editStorage.trim(),
      };
    });

    const nextLogs = appendRollingLog(
      vendorData?.logs,
      logChannel,
      {
        action: `Edited product: ${oldProduct['Device Type'] || 'Unknown Device'}`,
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-[#F9FAFB] min-h-screen">
      ...
      <div className="bg-white border border-gray-200 rounded-[12px] p-5 mb-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="border rounded-[10px] p-4 bg-gray-50">
            ...
            <button onClick={() => handleCopyLink(vendorBackendLink)} className="bg-blue-600 text-white px-4 py-2 rounded-[8px] text-sm font-bold hover:bg-blue-700 transition-colors shadow-sm">
              Copy Backend Link
            </button>
          </div>
          <div className="border rounded-[10px] p-4 bg-gray-50">
            ...
            <button onClick={() => handleCopyLink(customerStoreLink)} className="bg-green-600 text-white px-4 py-2 rounded-[8px] text-sm font-bold hover:bg-green-700 transition-colors shadow-sm">
              Copy Store Link
            </button>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-[12px] p-5 mb-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {THEME_PRESETS.map((p) => (
                <button key={p} onClick={() => setThemeColorInput(p)} className={`w-8 h-8 rounded-full border-2 shadow-sm transition-transform hover:scale-110 ${themeColorInput === p ? 'border-gray-900' : 'border-white'}`} style={{ backgroundColor: p }} />
              ))}
            </div>
          </div>
          <div>
        ...
        <div className="mb-6">
          <label className="block text-sm font-bold text-gray-700 mb-2">Staff WhatsApp Numbers</label>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {whatsappNumbersInput.map((val, idx) => (
              ...
              }} className="w-full p-3 border rounded-[8px]" placeholder={`Number ${idx + 1}`} />
            ))}
          </div>
        </div>
        <div className="mb-6">
          <label className="block text-sm font-bold text-gray-700 mb-2">Allowed Source Groups</label>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
        ...
        <button onClick={handleExport} className="bg-green-600 text-white px-6 py-3 rounded-[10px] shadow-md hover:bg-green-700 font-bold transition-all">
          Export CSV
        </button>
      </div>
      ...
      {isAdmin && (
        <div className="mb-4 flex flex-wrap gap-3 items-center bg-blue-50 p-4 rounded-xl border border-blue-100">
          ...
          <span className="text-sm font-bold text-blue-700">{selectedProductIndexes.length} selected</span>
        </div>
      )}
      ...
      <div className="overflow-x-auto bg-white shadow-lg rounded-[12px] border border-gray-200">
        <table className="min-w-full text-left">
          <thead className="bg-[#1A1C23] text-white">
            <tr>
              {isAdmin && <th className="p-4 w-[50px]"><input type="checkbox" checked={allVisibleRowsSelected} onChange={toggleSelectAll} className="w-4 h-4" /></th>}
          ...
          <tbody className="divide-y divide-gray-100">
            {displayData.map(({ product, index }) => (
              <tr key={index} className={`hover:bg-blue-50/30 transition-colors ${product.isVisible === false ? 'bg-gray-50 opacity-60' : ''}`}>
                {isAdmin && <td className="p-4"><input type="checkbox" checked={selectedProductIndexes.includes(index)} onChange={() => toggleProductSelection(index)} className="w-4 h-4" /></td>}
                ...
                <td className="p-4">
                  <button onClick={() => openEditModal(index, product)} className="bg-blue-600 text-white px-3 py-1.5 rounded-md text-xs font-bold hover:bg-blue-700">✏️ Edit</button>
                </td>
              </tr>
            ))}
          </tbody>
