...
const buildSpecificationParts = (product) => {
  const spec = product['SIM Type/Model/Processor'] || '';
  const storage = product['Storage Capacity/Configuration'] || '';
  return {
    specification: spec,
    storage,
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
          ...
          <tbody>
            {displayData.map(({ product, index }) => (
              ...
              >
                ...
                <td className="p-4">
                  <button
                    onClick={() => openEditModal(index, product)}
      ...
      <div className="bg-white border border-gray-200 rounded-[12px] p-5 mt-8 shadow-sm">
        ...
        <div className="mb-4 flex gap-3">
          <button
            onClick={() => setTimelineTab('vendor')}
            className={`px-4 py-2 rounded-lg font-semibold ${timelineTab === 'vendor' ? 'bg-[#1A1C23] text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            Vendor Logs
          </button>
          <button
            onClick={() => setTimelineTab('customer')}
            className={`px-4 py-2 rounded-lg font-semibold ${timelineTab === 'customer' ? 'bg-[#1A1C23] text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            Customer Logs
          </button>
          {isAdmin && (
            <button
              onClick={() => setTimelineTab('admin')}
              className={`px-4 py-2 rounded-lg font-semibold ${timelineTab === 'admin' ? 'bg-[#1A1C23] text-white' : 'bg-gray-100 text-gray-700'}`}
            >
              Admin Logs
            </button>
          )}
